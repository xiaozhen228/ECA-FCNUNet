...
def conv3x3_gn_relu(in_channel, out_channel, num_group):
    return nn.Sequential(
        nn.Conv2d(in_channel, out_channel, 3, 1, 1),  #卷积核大小、步长、填充
    ...
    layers = [
        nn.Sequential(
            ConvLSTM(input_channels=cl_channel, hidden_channels=[cl_channel, cl2_channel], kernel_size=5, step=8,
                     effective_step=[7]).to(device),   
            BasicBlock(gn_a), gn_relu(block_channel1, r), )]
    return nn.Sequential(*layers)   #改动了
...
class SSDGL(nn.Module):
    def __init__(self, config):
        ...
        self.cls_pred_conv = nn.Conv2d(self.config["in_channels"], self.config["num_classes"], 1)  #最后进行预测的一步

    def top_down(self, top, lateral):
        top2x = F.interpolate(top, scale_factor=2.0, mode='bilinear')  #这应该是一个上采样
        return lateral + top2x    #这还是一个相加的过程，所以通道数和大小都一定要一样

    def forward(self, x, y=None, w=None, **kwargs):    #他这个继承的类中默认执行这里前向传播函数
        feat_list = []   #这个x也应该被封装了，由dataloader来
        for op in self.feature_ops:   #看到了吗，这里就是一个前向传播过程
            x = op(x)

            if isinstance(op, nn.Identity):   #这是用来判断类型来用的
                feat_list.append(x)
        inner_feat_list = [self.reduce_1x1convs[i](feat) for i, feat in enumerate(feat_list)]  #这个应该是中间那个1*1的卷积

        inner_feat_list.reverse()    #翻一下，因为最后一个是第一个加的
        ...
        logit = self.cls_pred_conv(final_feat)  #预测值
        '''
        if self.training:
            loss_dict = {
                'cls_loss': self.loss(logit, y, w)     #计算损失函数，我觉得这个包把那些梯度清零与反向传播部分给封装了
            }
            return loss_dict
...
class ConvLSTM(nn.Module):    #调用时会自动向前传
    ...
    def forward(self, input):   #这个调用时也会自动的向前传
        internal_state = []
        outputs = []
