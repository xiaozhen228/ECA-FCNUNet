...
class FreeNet(nn.Module):
    def __init__(self, config):
        ...
        ])  #这里就是1×1的卷积过程，卷成了相同的通道数然后相加
        self.fuse_3x3convs = nn.ModuleList([
            nn.Conv2d(256, inner_dim, 3, 1, 1),
            nn.Conv2d(192+inner_dim, inner_dim, 3, 1, 1),
            nn.Conv2d(128+inner_dim, inner_dim, 3, 1, 1),
            nn.Conv2d(96+inner_dim, inner_dim, 3, 1, 1),
        ])
        self.cls_pred_conv = nn.Conv2d(inner_dim, self.config["num_classes"], 1)

    def top_down(self, top, lateral):
        top2x = F.interpolate(top, scale_factor=2.0, mode='nearest')   #什么类似于线性内插之类的
        #return lateral + top2x
        return torch.cat((lateral,top2x),dim=1)
    ...
    def forward(self, x, y=None, w=None, **kwargs):
        ...
        #inner_feat_list.reverse()   #这里得到应该是四个结果
        inner_feat_list = feat_list
        inner_feat_list.reverse() 

        out_feat_list = [self.fuse_3x3convs[0](inner_feat_list[0])]  #这里这一步没有理解
        for i in range(len(inner_feat_list) - 1):
            inner = self.top_down(out_feat_list[i], inner_feat_list[i + 1])   #我知道了，边往里面加边调用
