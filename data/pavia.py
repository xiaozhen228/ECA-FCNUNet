...
class NewPaviaDataset(FullImageDataset):
    def __init__(self,
        ...
        #=======================================
        if ynPCA:
            if os.path.exists('./PCAdata/pavia/PaviaU.mat'):
                im_mat = loadmat('./PCAdata/pavia/PaviaU.mat')
                image = im_mat['paviaU']
                print("加载PCApavia成功")
            else:
                print("进行paviaPCA")
            ...
            if not os.path.exists('./PCAdata/pavia/PaviaU.mat'):
                savemat("./PCAdata/pavia/PaviaU.mat",{'paviaU':image})
        else:
            im_mat = loadmat(image_mat_path)
            image = im_mat['paviaU']
